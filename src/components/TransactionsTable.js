...
import { Spinner, Table, Button, OverlayTrigger, Popover } from 'react-bootstrap';
...
export default function TransactionsTable({ contractToViewEvents }) {
    ...
    return (<>
        <h4 style={{ overflowWrap: 'break-word' }}>Contract: {contractToViewEvents}</h4>
        <Table striped bordered hover size="sm" responsive>
            ...
            {currentEvents ?
                <tbody>
                    {currentEvents.map((event) => <tr key={event.hash}>
                        <td>
                            <a
                        ...
                        <td>
                            <OverlayTrigger trigger="click" rootClose overlay={<Popover id="popover-basic">
                                <Popover.Header as="h3">Transaction Data</Popover.Header>
                                <Popover.Body>
                                    {event.trigger_info ? Object.entries(event.trigger_info).map(([key, value]) => <p key={event.hash+key}>{key} : {typeof value === 'string' ? value : Object.entries(value).toString()}</p> ) : 'no data!'}
                                </Popover.Body>
                            </Popover>}>
